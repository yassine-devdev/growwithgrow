...
import { CRM_SETTINGS_SECTIONS_L3 } from '../../../constants';
import * as Icons from './Icons';

interface SettingsL3SidebarProps {
    activeSection: CRMSettingsSection;
    setActiveSection: (section: CRMSettingsSection) => void;
}
...
const iconMap: Record<CRMSettingsSection, React.FC<{className?: string}>> = {
    'Pipeline': Icons.PipelineSettingsIcon,
    'Properties': Icons.PropertiesIcon,
    'Team': Icons.TeamIcon,
};

const SettingsL3Sidebar: React.FC<SettingsL3SidebarProps> = ({ activeSection, setActiveSection }) => {
    return (
        <aside className="w-20 flex-shrink-0 flex flex-col items-center justify-start py-4 px-2 h-full">
            <div className="bg-[#4a044e] rounded-2xl p-2 shadow-lg w-full h-full flex flex-col overflow-hidden">
                <nav className="w-full overflow-y-auto pr-1">
                    {CRM_SETTINGS_SECTIONS_L3.map(section => (
                        <NavItem 
                            key={section}
...
export default SettingsL3Sidebar;
