...
import { CRMSchoolSection } from '../../../types';
import { CRM_SCHOOL_SECTIONS_L3 } from '../../../constants';
import * as Icons from './Icons';

interface SchoolL3SidebarProps {
    activeSection: CRMSchoolSection;
    setActiveSection: (section: CRMSchoolSection) => void;
}
...
const iconMap: Record<CRMSchoolSection, React.FC<{className?: string}>> = {
    'All Schools': Icons.AllSchoolsIcon,
    'Add School': Icons.AddSchoolIcon,
    'Data Import': Icons.DataImportIcon,
    'Preferences': Icons.PreferencesIcon,
    'Billing & Subscriptions': Icons.BillingIcon,
    'Analytics': Icons.SchoolAnalyticsIcon,
};

const SchoolL3Sidebar: React.FC<SchoolL3SidebarProps> = ({ activeSection, setActiveSection }) => {
    return (
        <aside className="w-20 flex-shrink-0 flex flex-col items-center justify-start py-4 px-2 h-full">
            <div className="bg-[#4a044e] rounded-2xl p-2 shadow-lg w-full h-full flex flex-col overflow-hidden">
                <nav className="w-full overflow-y-auto pr-1">
                    {CRM_SCHOOL_SECTIONS_L3.map(section => (
                        <NavItem 
                            key={section}
...
export default SchoolL3Sidebar;
