...
import { CRM_CONTACTS_SECTIONS_L3 } from '../../../constants';
import * as Icons from './Icons';

interface ContactsL3SidebarProps {
    activeSection: CRMContactsSection;
    setActiveSection: (section: CRMContactsSection) => void;
}
...
const iconMap: Record<CRMContactsSection, React.FC<{className?: string}>> = {
    'All Contacts': Icons.ContactsIcon,
    'Companies': Icons.CompaniesIcon,
    'Lists': Icons.ListsIcon,
    'Import/Export': Icons.ImportExportIcon,
};


const ContactsL3Sidebar: React.FC<ContactsL3SidebarProps> = ({ activeSection, setActiveSection }) => {
    return (
        <aside className="w-20 flex-shrink-0 flex flex-col items-center justify-start py-4 px-2 h-full">
            <div className="bg-[#4a044e] rounded-2xl p-2 shadow-lg w-full h-full flex flex-col overflow-hidden">
                <nav className="w-full overflow-y-auto pr-1">
                    {CRM_CONTACTS_SECTIONS_L3.map(section => (
                        <NavItem 
                            key={section}
...
export default ContactsL3Sidebar;
