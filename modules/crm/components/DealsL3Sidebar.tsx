...
import { CRM_DEALS_SECTIONS_L3 } from '../../../constants';
import * as Icons from './Icons';

interface DealsL3SidebarProps {
    activeSection: CRMDealsSection;
    setActiveSection: (section: CRMDealsSection) => void;
}
...
const iconMap: Record<CRMDealsSection, React.FC<{className?: string}>> = {
    'Pipeline View': Icons.PipelineViewIcon,
    'List View': Icons.ListViewIcon,
    'Forecasting': Icons.ForecastingIcon,
};

const DealsL3Sidebar: React.FC<DealsL3SidebarProps> = ({ activeSection, setActiveSection }) => {
    return (
        <aside className="w-20 flex-shrink-0 flex flex-col items-center justify-start py-4 px-2 h-full">
            <div className="bg-[#4a044e] rounded-2xl p-2 shadow-lg w-full h-full flex flex-col overflow-hidden">
                <nav className="w-full overflow-y-auto pr-1">
                    {CRM_DEALS_SECTIONS_L3.map(section => (
                        <NavItem 
                            key={section}
