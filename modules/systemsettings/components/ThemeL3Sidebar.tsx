...
import { SYSTEM_SETTINGS_THEME_SECTIONS_L3 } from '../../../constants';
import * as Icons from './Icons';

interface ThemeL3SidebarProps {
    activeSection: SystemSettingsThemeL3Section;
    setActiveSection: (section: SystemSettingsThemeL3Section) => void;
}
...
const iconMap: Record<string, React.FC<{className?: string}>> = {
    ...
    'Effects': Icons.EffectsIcon,
    'Accessibility': Icons.AccessibilityIcon,
};

const ThemeL3Sidebar: React.FC<ThemeL3SidebarProps> = ({ activeSection, setActiveSection }) => {
    return (
        <aside className="w-20 flex-shrink-0 flex flex-col items-center justify-start py-4 px-2 h-full">
            <div className="bg-[#475569] rounded-2xl p-2 shadow-lg w-full h-full flex flex-col overflow-hidden">
                <nav className="w-full overflow-y-auto pr-1">
                    {SYSTEM_SETTINGS_THEME_SECTIONS_L3.map(section => (
                        <NavItem 
                            key={section}
